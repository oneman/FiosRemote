...
while true 
  sockets = [socket]
  readable, writeable = IO.select(sockets, nil ,nil,99)
  ...
  if sock == readable
    begin 
         ...
         if data
           puts "Got some data: #{Time.now}"
           puts data.unpack("H*")
         end
    rescue Errno::EAGAIN, Errno::EWOULDBLOCK
      puts "read would have bocks"
    end 
  end
  end
  ...
  readable.each do |sock| 
    begin 
      ...
      puts "write would have bocks"
    end 
  end
end

=begin
puts "Entering loop recieve state"
while true
 puts "Looped"
...
doable = Kernel.select([readsock],[writesock],nil)

puts "monkey"

if doable != nil && doable[0] != nil



 data = sock.recvfrom_nonblock( 2220 )[0].chomp
 if data
  puts "Got some data: #{Time.now}"
  puts data.unpack("H*")
 end
...
# sock.write [construct_fios_remote_packet_exp().join ''].pack('H*')
# puts "Expecting / printing response"
# data = sock.recvfrom( 2220 )[0].chomp
# if data
#  puts data
